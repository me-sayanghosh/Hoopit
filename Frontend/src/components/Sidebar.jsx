    ...
    <aside className="hidden lg:flex w-64 flex-col px-2 py-6 sticky top-6 self-start h-[calc(100vh-48px)] overflow-auto">
      <div className="rounded-3xl bg-white shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-slate-200/80 p-4 flex flex-col h-full">
        <div className="flex items-center gap-3 px-2 mb-2">
          ...
          <div className="ml-auto">
            <button
              ...
              title="View Profile"
              className="group relative transition-transform hover:scale-105 active:scale-95 focus:outline-none flex items-center"
            >
              <img
                src={profile?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile?.name || 'User')}`}
                alt="avatar"
                className="h-8 w-8 rounded-full object-cover ring-2 ring-slate-100 group-hover:ring-blue-200 shadow-sm transition"
              />
            </button>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-2xl bg-slate-50/50 border border-slate-100 p-4 shadow-sm shrink-0">
          ...
          <div className="mt-5 text-xs font-bold uppercase tracking-wider text-slate-400">Account</div>
          <nav className="mt-2 space-y-1">
            <NavItem label="Profile Settings" icon="cog" to="/profile" />
          </nav>
        </div>
